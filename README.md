...
If you expect only strings or positive numbers (specifying `Number` in the constraints is redundant because `Expect.positive` implies this):
```javascript
var num = getSomeNumber();
Expect(num, String, Expect.positive);
```
...
Expect.array(myArray, String, Expect.positive, Boolean); // Fails
...
// Allow only numbers, booleans, and non-empty strings
Expect.object(myObject, Number, Boolean, [String, Expect.notEmpty]); // Passes
```

##Value checking functions

You can call these functions directly. Or, you can pass them into a constraint (but for some this is silly, i.e. you can just pass `String` and `Function` into a constraint instead of passing in `Expect.string` or `Expect.func`).
